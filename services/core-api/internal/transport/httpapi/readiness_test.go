...
func appWithStore(t *testing.T, store *stubStore, logs *bytes.Buffer) *fiber.App {
	t.Helper()
...
func TestDrainingReportsNotReadyWithoutConsultingTheStore(t *testing.T) {
	...
	readiness := &httpapi.Readiness{}
	app, err := httpapi.New(httpapi.Options{
		Logger: logger, Readiness: readiness, RateLimit: directPolicy(1000),
...
func TestTheConstructorRefusesAServiceWithNoStoreBehindIt(t *testing.T) {
	base := httpapi.Options{
		Logger: slog.New(slog.NewJSONHandler(&bytes.Buffer{}, nil)), Readiness: &httpapi.Readiness{},
		RateLimit: directPolicy(10), Persistence: newStubStore(true), CheckTimeout: time.Second,
		ReadTimeout: time.Second, WriteTimeout: time.Second, IdleTimeout: time.Second,
	}
