...
func TestClientErrorMessageIsPreservedButServerErrorIsNot(t *testing.T) {
	...
	var client httperror.ErrorResponse
	...
	var server httperror.ErrorResponse
...
func TestRequestLogRecordsTheStatusActuallyReturned(t *testing.T) {
	...
	readiness := &healthapi.Readiness{}
	readiness.Set(true)
	app := httpfixture.MustApp(t, &httpapi.Options{Logger: slog.New(slog.NewJSONHandler(&captured, nil)), Readiness: readiness, RateLimit: httpfixture.DirectPolicy(1000)})
