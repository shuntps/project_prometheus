...
func TestLoaderCanonicalisesEverySupportedProxyHeader(t *testing.T) {
	for _, canonical := range ratelimit.SupportedProxyHeaders() {
		for _, spelling := range []string{canonical, lower(canonical), "  " + canonical + "  "} {
			...
			if _, err := httpapi.New(completeExcept(cfg.RateLimit)); err != nil {
...
// completeExcept supplies every dependency unrelated to the network policy, so a
// refusal can only come from the policy the case is actually proving.
