...
// sessionRoles reads the public session view the running service serves and
// returns the roles array exactly as the JSON carries it.
func sessionRoles(t *testing.T, base string, cookie *http.Cookie) []string {
	t.Helper()
	...
	if res.StatusCode != http.StatusOK {
		raw, _ := io.ReadAll(res.Body)
		t.Fatalf("the session route returned %d, want 200: %s", res.StatusCode, raw)
	}
	var view struct {
...
// TestTheServedRolesCarryTheApplicationOrder grants the roles in the reverse of
// the order the response must carry, so the database cannot supply that order.
func TestTheServedRolesCarryTheApplicationOrder(t *testing.T) {
	dsn, host := realPostgres(t)
	target := dsnFor(t, dsn, host)
	applySchema(t, target)
	...
	const address, plaintext = "ordered@example.com", "correct-horse-battery-staple"
	encoded, err := hasher.Hash(plaintext)
	if err != nil {
		t.Fatalf("hashing failed: %v", err)
	}
	...
	account, err := store.CreateAccount(context.Background(), authstore.NewAccount{
		Kind:     iam.KindCreator,
		Status:   iam.StatusActive,
		Email:    email,
		Password: encoded,
		Roles:    []iam.Role{iam.RoleViewer, iam.RoleCreator},
	}, time.Now().UTC())
	if err != nil {
		t.Fatalf("creating the account failed: %v", err)
	}

	// The grants are stored the other way round, which is what the statement
	// returns now that no database order is asked for.
	var stored []string
	if err := pool.QueryRow(context.Background(),
		`SELECT ARRAY(SELECT r.role FROM account_role_grants r WHERE r.account_id = a.id) FROM accounts a WHERE a.id = $1`,
		account.ID.String()).Scan(&stored); err != nil {
		t.Fatalf("reading the stored grants failed: %v", err)
	}
	if want := []string{"viewer", "creator"}; !sameSequence(stored, want) {
		t.Fatalf("the fixture stored %v, want %v so the assertion below is not vacuous", stored, want)
	}
	...
	cookie, _ := signedInClient(t, base, address, plaintext)
	served := sessionRoles(t, base, cookie)
	if want := []string{"creator", "viewer"}; !sameSequence(served, want) {
		t.Fatalf("the served roles are %v, want %v", served, want)
	}
}

// sameSequence compares position by position. The standard library's slices
// package cannot be imported here: this package already declares that name.
