...
func TestNoClientHeaderDecidesTheIdentity(t *testing.T) {
	s := newSurface(t)
	viewerAddress, viewerAccount := s.account(t, iam.KindViewer, iam.StatusActive, iam.RoleViewer)
	_, creatorAccount := s.account(t, iam.KindCreator, iam.StatusActive, iam.RoleViewer, iam.RoleCreator)
	...
	spoofed := []struct{ header, value string }{
		...
		{"X-Account-Kind", string(iam.KindCreator)},
		{"X-Role", string(iam.RoleCreator)},
		{"X-Roles", string(iam.RoleOperatorFinance)},
		{"X-Surface", string(iam.SurfaceOperator)},
		{"X-Permission", string(iam.PermissionStreamBroadcast)},
	...
	if view["kind"] != string(iam.KindViewer) || view["surface"] != string(iam.SurfacePublic) {
