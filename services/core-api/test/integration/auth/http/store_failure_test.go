...
func (f *faultyStore) RecordActivity(ctx context.Context, id session.ID, now time.Time, lifetimes session.Lifetimes) (bool, error) {
	if f.activity != nil {
...
func (f *faultyStore) RevokeSession(ctx context.Context, id session.ID, now time.Time) (bool, error) {
	if f.revoke != nil {
